...
def test_anilibria_browse_with_page(client, auth_header):
    ...
    resp = client.get('/api/anilibria/browse?page=1', headers=auth_header)
    assert resp.status_code == 200
...
def test_anilibria_detail_by_code(client, auth_header):
    ...
    resp = client.get('/api/anilibria/detail?code=naruto', headers=auth_header)
    # May return 200 or 502 depending on API availability
