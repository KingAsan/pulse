...
class HdRezkaService:
    ...
    def get_episode_stream(self, url, season, episode, translator_id=''):
        """Get video stream for specific season/episode via HDRezka AJAX API.
        
        This is the correct way to get episode-specific streams according to 
        HdRezkaApi library implementation.
        
        Returns dict with video URLs by quality.
        """
        if not url:
            return None
            
        try:
            # Get page to extract post_id
            soup = self._get(url)
            if not soup:
                return None
            ...
            if not post_id:
                ...
                if data_id_el:
                    post_id = data_id_el.get('data-id', '')
            
            if not post_id:
                return None
            
            # Get translator_id from page if not provided
            ...
            # Make AJAX request to get stream (like HdRezkaApi does)
            ajax_url = f'{self.base_url}/ajax/get_cdn_series/'
            data = {
                'id': post_id,
            ...
            r = self.session.post(ajax_url, data=data, timeout=15)
            if r.status_code != 200:
                logger.warning(f'AJAX request failed: {r.status_code}')
                return None
            
            response = r.json()
            if not response.get('success') or not response.get('url'):
                logger.warning('AJAX response unsuccessful')
                return None
            
            # Decode the base64-encoded stream URL
            encoded_url = response['url']
            decoded = self._decode_stream_url(encoded_url)
            ...
            logger.exception('get_episode_stream error: %s', url)
            return None
    
    def _decode_stream_url(self, encoded_url):
        """Decode HDRezka stream URL (base64 with trash removal)."""
        import base64
        from itertools import product
        ...
        try:
            ...
            for i in range(2, 4):
                ...
                for chars in product(trash_list, repeat=i):
                    ...
                    trash_combo = base64.b64encode(data_bytes)
                    trash_codes_set.append(trash_combo)
            
            arr = encoded_url.replace("#h", "").split("//_//")
            ...
            padding = 4 - (len(trash_string) % 4)
            if padding != 4:
                trash_string += "=" * padding
            
            decoded = base64.b64decode(trash_string).decode('utf-8')
            return decoded
        except Exception as e:
            logger.exception('Error decoding stream URL')
            return encoded_url
